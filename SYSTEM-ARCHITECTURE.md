...
- **Clipboard History 是同级独立窗口与持久化子系统**：`⌘⇧P` 只在当前输入源属于 RIMES 时打开屏幕底部的 nonactivating 窗口，不显示或改变 Buffer；切到其他输入法会立即撤销该窗口的搜索与激活权限。收录开启且 secure input、锁屏、睡眠、会话失活均不存在时，同一 RIMES 进程在后台读取 pasteboard，将文本、链接、图片、文件、颜色与未知类型的原始 representation 持久写入 `~/Library/Application Support/RIMES/clipboard/clipboard.sqlite`；保护期间不读，恢复只更新 `changeCount` baseline，不补抓。原始负载惰性读取，不写正文日志或仓库；拒绝 confidential/transient 标记。横向卡片支持全库元数据与图片类别别名搜索；单击只改变选择，双击、Return 或 `⌘1`–`⌘9` 才激活，Shift 加纵向滚轮按反向映射横向移动。文本/链接在 exact `FocusToken` 复验后经 IMK `Delivery.insert` 上屏，成功后提升到历史首位并关闭。图片、RTF、HTML、文件、颜色与未知类型只把原始 representations 无损恢复到 `NSPasteboard.general`、提升到历史首位并静默关闭，用户随后自行按 `⌘V`。Clipboard 绝不调用 Paste 或右键菜单，不合成、发送或定向投递 `⌘V`，也不请求 Accessibility/Post Event。`⌘C` 仍把所选原始表示复制到系统剪贴板并提升历史，但保持窗口打开；Delete 持久删除，`Esc` 先清搜索再关闭。Paste 导入是只读、幂等、可审计的本地迁移路径；历史、原始负载与预览仅在本机，不进仓库、不做云端或跨设备同步。
- **Capsule 是同级本机内容模块**：Capsule 在 `~/Library/RimeBuffer/capsule/entries/*.md` 用 Obsidian 可读 front matter + 正文逐条管理 Prompt、Memory、Note、HTTP(S) URL，以及 Skill/Image/PDF 绝对路径，并在第一次初始化时只预设一次标题为 `RIMES 默认词条`、正文为 `RIMES` 的 Memory。`capsule/content-library-v1.json` 保存本机库身份；同步配置与逐库 revision/fingerprint 状态分别位于同级 `capsule-sync/config-v1.json` 和 `capsule-sync/state-<library-id>.json`。Image 使用 ImageIO 后台生成有界缩略图；PDF 使用 Core Graphics 在全局串行队列中只渲染第 1 页的有界缩略图并显示总页数。手工修改路径会立即作废旧请求，任何迟到结果还需通过 generation、类型和路径复验。Password 使用独立密文目录：`passwords/*.md` 仅暴露标题/UUID/更新时间，网址、App、用户名、当前密码和曾用密码均为 ChaCha20-Poly1305 密文。可输入的 `CapsuleWindowController` 由 `⌘⇧C` 全局开关；`CapsulePaneViewController` 按八类搜索并完整增删改查。可选的 `CapsuleCloudSyncController` 让用户选择真实 iCloud Drive 文件夹，以逐条 Markdown、tombstone、冲突副本和 SHA-256 媒体资产做后台双向 reconcile；本机目录仍是 canonical。同步只包含 Prompt、Memory、Note、URL、Image 与 PDF；Skill 绝对路径、Password 与 `master-key` 永不进入云镜像。下载的媒体校验后 materialize 到 `capsule/assets/`；本机媒体暂时不可读且本轮需要上传时，该 UUID 进入 deferred，其他条目继续同步，不据此推断删除。离线媒体败者在正式 entry 被覆盖或删除前先完整写入本机私有 `capsule/conflicts/`，云端只留脱敏摘要；托管缓存意外丢失时从仍有效的 asset 自愈。删除 Image/PDF 条目只删除 entry 并写 tombstone；为保留跨设备并发和冲突恢复能力，本轮不会自动回收已上传的内容寻址 asset 或本机 materialized cache。仍被有效条目或 `conflicts/` 引用的 asset 不应手工删除。Capsule 不进入 Buffer catalog、插件启停、派生 workspace、Return 投递或飞耀并击路径；当前窗口是管理面，不是外部投递目标。
- **Mailbox 是同级本机会话模块**：`⌘⇧M` 打开的 Mailbox 使用本地持久化会话，跨输入法进程重启保存；Buffer 正文只属于当前进程，Clipboard History 则使用自己的本机持久库，两者都不因 Mailbox 引入发送历史。会话正文使用固定列、全宽、等宽的 CLI transcript，角色只改变提示符和弱强调，不改变左右位置或正文宽度。左栏“新建对话”进入进程内草稿态，不预先持久化空会话；模型选择器只列当前已配置的连接器/模型，Codex CLI 与 Claude Code CLI 分别只暴露各自默认模型，OpenAI 兼容 API 暴露本机私有配置中的 model。该选择在首次 Return 时冻结为会话级快照，不修改全局连接器选择；同一次动作原子创建会话、持久化首个用户 turn 并预留后台 generation，随后按冻结的连接器/模型发起生成。MCP/HTTP 等单向推送先进入 Mailbox 审核；查看、备注和 AI 续问不依赖 Buffer，“发送到 Buffer / 拒绝”由可选桥接协调器提供，不属于 Mailbox Store 或窗口生命周期。
...
- **全局切换快捷键与 Esc**：`GlobalHotKeyController` 用 Carbon 注册精确且可配置的 `Command+Shift+B`、`Command+Shift+P`、`Command+Shift+M` 与 `Command+Shift+C`，不需 Accessibility 权限。B 调用 `BufferWindowController.toggleVisibility()` 并沿用 Buffer 的 open/resume 与 close/pause 生命周期；P 调用 `ClipboardHistoryWindowController.toggleVisibility()`，打开或关闭屏幕底部的独立 nonactivating 窗口，不恢复 Buffer 捕获；M 与 C 分别开关 Mailbox、Capsule。注册快捷键均被消费，不继续传给前台应用。Buffer 的 `Esc` 只关闭 Buffer；Clipboard 搜索非空时 `Esc` 先清搜索，搜索为空时关闭 Clipboard 自己，始终不触碰宿主 composition。
...
- **关闭不会删除已有块**：先显式收束当前组字，折叠工具栏，暂停捕获，结束 transient 加载/错误状态并保留已有模型块，再隐藏。从设置/输入法菜单显示工作台时会恢复底层捕获。工作台没有手动清空或撤销入口；隐私选项触发的跨 app 清理仍是不可恢复的安全操作。
...
- **MailboxWindowController / MailboxPaneViewController**：独立可编辑会话窗口与设置页复用双栏 CLI transcript；共享本地 Store 选中态，关闭时不恢复 Buffer。独立窗口的“新建对话”先维护进程内草稿与会话级模型选择，首次 Return 才通过 `AITextMailboxGenerationCoordinator` 创建持久会话并启动生成，因此取消或关闭草稿不会留下空会话，也不会改写全局连接器偏好。
...
Sources/RimeBuffer/
  ...
  CapsuleWindowController.swift   八类本机 Markdown/密文条目 CRUD、媒体预览与独立可输入窗口
  ...
  ClipboardHistoryWindowController.swift 独立底部 nonactivating 窗口、文本 exact-token 投递、富内容 pasteboard 恢复与会话保护
  ...
  Mailbox/
    ...
    MailboxPaneViewController.swift  设置页与独立窗口共用双栏 UI、进程内新对话草稿与模型选择
...
| **缓冲窗口** | FocusToken / 打开即逻辑聚焦 Buffer / 初始44/78pt + 前置图标切换33+1pt工具栏至78/112pt / 工具栏空白拖动 / 无专用右侧拖动条 / target 无角色图标 / 生成结果复制与纯 Cmd+C / Return+Backspace 隔离 / inline preedit + 独立候选浮窗 / 成功块无历史消费 / 多屏与隐私 | ✅ 源码 smoke 已覆盖；待安装后真实宿主输入交互验收 |
