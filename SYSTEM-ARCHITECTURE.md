...
- **Capsule 是同级本机内容模块**：Capsule 在 `~/Library/RimeBuffer/capsule/entries/*.md` 用 Obsidian 可读 front matter + 正文管理 Prompt、Memory 与 Skill 绝对路径，并在第一次初始化时只预设一次标题为 `RIMES 默认词条`、正文为 `RIMES` 的 Memory。Password 使用独立密文目录：`passwords/*.md` 仅暴露标题/UUID/更新时间，网址、App、用户名、当前密码和曾用密码均为 ChaCha20-Poly1305 密文。可输入的 `CapsuleWindowController` 由 `⌘⇧C` 全局开关；`CapsulePaneViewController` 按四类搜索并完整增删改查。Capsule 不进入 Buffer catalog、插件启停、派生 workspace、Return 投递或飞耀并击路径；当前窗口是管理面，不是外部投递目标。
...
Sources/RimeBuffer/
  ...
  CapsuleWindowController.swift   四类本机 Markdown/密文条目 CRUD 面板与独立可输入窗口
...
`clipboard-history-smoke` 覆盖独立窗口显隐、关闭窗口后继续后台收录、收录关闭时零 pasteboard 读取、全 representation 有界归档、后台采集与保护切换竞态、去重与字节/条目上限、confidential/transient marker 拒绝、secure/锁屏/睡眠/会话保护即时遮蔽、恢复只建 baseline 且不补抓、SQLite 重开与退出前写队列 flush，以及横向卡片、元数据/图片别名搜索、异步有界图片缩略图、图片文件预览、真实来源 App 图标、左右选择、纯文本 exact-token 投递、富内容无损重建、定向宿主粘贴事件的标签/键形、`Command+C` 自收录抑制、Delete 与 Esc 行为。真实跨进程 Post Event 权限与宿主粘贴仍需安装态人工回归。`buffer-window-smoke` 独立覆盖 Buffer 折叠/展开几何、candidate `0` 放行与 1–9/齿轮边界；Clipboard 不再进入 BufferModel 或影响其 canonical frame。`mailbox-store-smoke` 覆盖 0600/0700、原子写入、容量、旧消息缺失格式字段、Plain/Markdown/JSON 持久化、稳定且不落盘的 generation preview、跨重启中断、未读、持久审核、重复决策与 InboundBus 隔离/恢复；其 UI 部分还覆盖原生新会话草稿不落盘、CLI 默认模型/OpenAI 配置模型目录、连接器异步可用性刷新、失败文案保留、延迟完成期间导航锁和模型选择冻结。`mailbox-toast-smoke` 固定完成事件、流式 preview 零通知及自动已读竞态；`ai-text-mailbox-smoke` 覆盖退役 Mailbox 偏好的缺失/损坏/合法 v2 迁移矩阵、持久化幂等性、真实 AppKit 输出菜单仅有 Plain/Markdown/JSON、AI 生成仅走 inline 路由、inline 格式校验，以及 Mailbox 原生首轮生成、真实续问、后台终态与单向本地备注。
