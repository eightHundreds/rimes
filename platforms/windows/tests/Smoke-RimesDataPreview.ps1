...
$expectedPayloadFiles = @((Get-Content -LiteralPath $previewPolicy -Raw -Encoding UTF8 | ConvertFrom-Json).include).Count
Assert-PreviewTest -Condition ($expectedPayloadFiles -ge 1) -Message 'policy.json include list is empty'
...
try {
    ...
    Assert-PreviewTest -Condition ([int]$buildResult.PayloadFiles -eq $expectedPayloadFiles) -Message "reviewed payload is not the expected $expectedPayloadFiles-file policy.json closure"
    ...
    Assert-PreviewTest -Condition ($payloadFiles.Count -eq $expectedPayloadFiles) -Message "extracted ZIP does not contain exactly $expectedPayloadFiles payload files (policy.json include)"
