...
> **2026-08-30 Buffer UI、结果复制与主题最新覆盖（优先于本文全部旧 UI 描述）**：Buffer 初始为单行紧凑态。普通/source-only/target-only 折叠为 44pt，live source+target 折叠为 78pt；点击正文唯一的前置输入/插件图标，切换恢复的 33pt 顶部工具栏及 1pt 分隔线，对应展开高度为 78/112pt。工具栏承载插件选择、当前插件配置、按状态出现的返回编辑、状态与关闭；空白 chrome、状态、间距和弹性留白是拖窗区，控件保持首击交互。右侧专用 24pt 拖拽条已删除。展开态只在当前进程/会话生效，不写偏好，并在隐藏或 secure/session protection 时重置折叠。alternative 数量与独立候选浮窗均不增高；target rail 不显示角色图标。完整且新鲜的非 `BufferModel` 生成结果显示复制按钮，纯 `Command+C` 与点击按钮等价：冻结并重新验证同一 source/workspace/generation/有序 block IDs 与文本，按 block 顺序无分隔拼接后写入系统剪贴板，再走安全关闭/暂停生命周期。复制不是上屏，不调用 `prepare`、`consume`、`BufferDeliveryCoordinator` 或 `Delivery.insert`，也不提交、修改或消费 source；普通 Buffer 原文、空/未完成、protected 或 stale/drifted 结果 fail closed/按既有规则透传。墨竹、翡翠、静谧是 **Classic** 主题家族的三种配色；**Rasta** 是独立、完整的红黄绿语义主题。本文后续 toolbarless、非持久弹层、专用右侧拖动区、永久展开工具栏、旧高度、target 角色图标、旧主题分类及通过 Delivery 复制等冲突表述均为历史记录，由本条覆盖。

> **2026-08-30 Buffer 点击、指针与拖动补充**：前置输入/插件图标在祖先 rail 手势识别前被排除，点击只切换顶部工具栏，不同时触发正文捕获；所有可交互按钮、弹出控件和可选 chip 在 nonactivating panel 内主动设置 pointing-hand，禁用或离开时恢复 arrow。只有展开工具栏的空白 chrome、状态、间距和弹性留白使用 open/closed-hand 并可拖窗；正文轨、窗口背景、插件/配置控件、复制、发送与关闭命中区都不参与拖动，不保留右侧 24pt 专用拖动区。
...
>
> **2026-08-30 当前模块边界**：Buffer、Clipboard History、Mailbox 与 Capsule 是四个同级核心本机模块，各自拥有独立窗口、快捷键和生命周期；当前 Buffer 只维护并发布 **AI 生成 2.1、实时翻译 2.1、意识流输入 1.3** 三个插件。Capsule 在 `~/Library/RimeBuffer/capsule` 用 Obsidian 可读 Markdown/本机密文管理 Prompt、Memory、Password 与 Skill，并一次性预设 `RIMES 默认词条` Memory；它不进入 Buffer 插件目录、派生 workspace、Return 投递或按键并击路径。Mailbox 可独立收信、保存会话与继续 AI 对话；“发送到 Buffer”仅是显式可选桥接。Clipboard History 不属于 Buffer，也不会把历史加入 `BufferModel`。`Marine Chrome`、`My Prompt`、`Remarkable` 已从当前产品清单下架，不再下载、安装、启用或出现在运行时路由中；本文后续关于它们的实现、版本和测试说明仅保留为历史设计记录，不代表当前可用能力。
>
...
>
> **2026-08-30 Clipboard History 持久化与图片预览覆盖（替代早先的 visible-only 纯文本方案）**：`Command+Shift+P` 通过可配置的全局 Carbon hot key 打开屏幕底部的独立 nonactivating 窗口，不显示 Buffer 外壳、不启停 `BufferModel`，也不参与 Buffer 几何。收录开关开启且 secure input、锁屏、睡眠、会话失活均不存在时，同一输入法进程在后台读取系统 pasteboard 并将文本、链接、图片、文件、颜色以及未知类型的原始 representation 持久写入 `~/Library/Application Support/RIMES/clipboard/clipboard.sqlite`；原始大负载只在用户激活时惰性读取，不写正文日志或仓库。带 confidential/transient 标记的内容和保护期间的内容仍拒绝收录，恢复时只建立新的 `changeCount` baseline，绝不补抓。界面借鉴 Paste 的横向卡片时间流：所有元数据和图片类别别名参与搜索、展示层最多实例化 200 张卡片；图片记录与可解码的图片文件异步请求有界缩略图，卡片同时显示真实来源 App 图标。文本/链接经 exact `FocusToken` 和 `Delivery.insert` 直接上屏；富内容先无损恢复原始 pasteboard，再向同一 token 对应的精确宿主 PID 投递一对带进程内标签的 `Command+V`，RIMES 自身只为这对事件旁路 Buffer 粘贴路由。该显式动作按需请求 macOS Post Event 权限，权限、PID、client 或 token 任一失配就 fail closed、保持窗口，不向其他焦点粘贴。`Command+C` 只恢复原始内容，Delete/清空同步更新持久库。Paste 导入使用来源标识幂等 upsert，保留每条原始压缩负载、时间、来源和类型，不修改 Paste 源数据。历史、原始负载与预览均只在本机，不进仓库、不加入 Buffer，也不做云端或跨设备同步。
>
...
>
> **2026-08-30 工作台焦点与候选承载覆盖**：用户显式从隐藏态唤出缓冲工作台时，先收束宿主直输组字，再把当前精确外部 `FocusToken` 的逻辑输入权交给 Buffer；宿主文本框只保留为后续投递锚点。随后以 `liveTarget` 的 controller/client、前台 PID 和 secure-input 门控前后夹住一次 caret 查询。`attributes(forCharacterIndex:)` 的下标按 InputMethodKit 契约相对于 inline session，必须沿用稳定的相对下标 `0`；宿主的 `selectedRange`/`markedRange` 是文档级坐标，禁止直接混入。合法零宽 caret 所在屏幕有空间时工作台优先位于输入框下方，否则翻到上方；只有 Buffer 自身的基础轨道参与最大高度和稳定方向预判，真实当前 frame 始终贴输入行 10pt。工作台自身高度变化沿远离输入框的方向展开。直输时唯一 `CandidateWindow` panel 跟随宿主 caret；Buffer 捕获时 Rime preedit 在 `BufferInlineView` 的逻辑插入光标处内联投影，同一个候选 panel 保持独立、nonactivating，并悬浮跟随该 Buffer caret，不占工作台高度。无精确目标、矩形非法或离屏时，才在鼠标所在屏幕居中靠下。工作台定位只发生在显式 hidden→visible 转换或跨 Space/显示器恢复，显示期间不会随同屏字段或流式刷新跳动；候选 panel 则随当前可见的真实输入 caret 更新。Clipboard History 拥有自己的底部窗口、frame 与输入路由，不参与本段 Buffer 定位。
>
...
- 工作台是独立、nonactivating、可调整宽度的 `NSPanel`。它初始只显示单行正文：普通/source-only/target-only 为 44pt，实时翻译与意识流的 live source+target 为 78pt。点击正文唯一的前置输入/插件图标，切换恢复的 33pt 顶部工具栏和 1pt divider；展开后对应为 78pt 与 112pt。工具栏显示状态、带小图标的缓冲插件选择器及当前配置、按状态出现的返回编辑与关闭；其空白 chrome、状态、间距和弹性留白调用 `performDrag`，按钮、下拉与其他 `NSControl` 保持首击。正文轨、窗口背景与右侧动作均不可拖，并且不再存在 24pt 专用右侧拖动条。展开态只属于当前进程/会话，不写 UserDefaults；隐藏、secure input、锁屏、睡眠或会话失活会重置折叠。派生 workspace 可提供 1–5 个 alternatives，但只维护一个稳定 target viewport，以 pager 切换而不改变当前折叠/展开高度；活动项内部的横向 document row 关闭 autoresizing-mask constraints 并将高度绑定到 scroll viewport。内置 AI 被选中时，右侧主控件在禁用 AI 图标/可请求 AI 图标/转圈/纸飞机间原位切换，工具栏不放第二个生成按钮。设置页可多选启用缓冲插件，选择器只显示该集合并用 `Default` 表示无插件；选择器直接改写唯一 owner。显式从隐藏态唤出时，先收束宿主直输组字，再把当前精确外部 `FocusToken` 的逻辑输入权交给 Buffer；外部文本框只保留为后续上屏目标。工作台读取该租约的一次新鲜 host caret 行矩形，只以 Buffer 自身最大 112pt 高度选择稳定一侧，真实当前高度仍与输入行保持 10pt；通常位于下方，空间不足则翻到上方。焦点锚定布局切 owner、单/双轨或工具栏状态时保持靠输入框的一侧并向外增减；手动/无目标布局仍固定底边。Rime preedit 直接占用 source rail 中现有的逻辑插入位置，候选 panel 独立计算自己的尺寸和屏幕夹取，因此两者都不改变工作台高度或 canonical frame。没有可信目标时才在鼠标所在屏幕居中靠下，显示后工作台不追踪光标。圆角表面使用当前主题 palette，内缩到透明窗口边距，并按 backing scale 在路径内绘制 hairline。显隐、frame 与 pin 持久化，多屏变化时恢复到可见区域；工具栏展开态和候选位置不持久化。普通关闭会收束组字、折叠工具栏、暂停捕获、结束 transient 加载/错误状态并保留已有块。`Command+Shift+B` 通过全局 Carbon hot key 调用 `toggleVisibility()`，隐藏态打开时优先把输入路由切到 Buffer；工作台不提供块编辑器、面板内缓冲开关、手动遮蔽、历史恢复或清空撤销。Clipboard History 的底部窗口不参与这里的尺寸、frame 或输入 ownership。
...
└── Sources/RimeBuffer/
    ...
    ├── ClipboardHistoryWindowController.swift ✅ 独立底部 nonactivating 窗口、exact FocusToken 投递与会话保护
    ...
    ├── CapsuleWindowController.swift ✅ 四类条目 CRUD 面板与独立管理窗口
