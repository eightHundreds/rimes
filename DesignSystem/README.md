...
- Clipboard History 不是 Buffer rail。`⌘⇧P` 打开屏幕底部的 nonactivating 独立窗口；收录开启且无 Secure Input、锁屏、睡眠或会话失活保护时，同一 RIMES 进程将文本、链接、图片、文件及其他原始 representation 持久保存在本机私有 SQLite。图片记录与可解码的图片文件异步加载有界缩略图，卡片显示真实来源 App 图标，图片类别别名也参与搜索。交互借鉴 Paste 的横向卡片时间流：直接输入搜索、左右选择、Return / `⌘1`–`⌘9` 激活、`⌘C` 复制、Delete 删除，Esc 先清搜索再关闭。历史、原始负载和预览不进仓库，也不做云端或跨设备同步。
...
- Buffer 初始为单行紧凑态：普通/source-only/target-only 折叠为 44pt，live source + target 折叠为 78pt。唯一前置输入/插件图标切换恢复的 33pt 顶部工具栏和 1pt divider，展开后分别为 78pt 与 112pt。工具栏承载插件选择、当前插件配置、返回编辑、状态与关闭；展开态不持久化，并在隐藏或 Secure Input/会话保护时折叠。目标轨不显示角色图标。
...
以上是 `design/react-system` 分支的 React 设计契约。后续调整必须让 React 设计源与 AppKit 实现同时遵守 1–5 项 pager、44/78pt 折叠与 78/112pt 展开几何、单输入/插件图标工具栏切换、工具栏拖动边界、状态单一承载、固定复制动作和 22×22 主操作；Clipboard 母版也必须呈现真实图片预览与来源 App 图标。
...
- Buffer 与 Clipboard 是两个独立窗口。Clipboard 的 React 母版只模拟交互；原生端仍以 nonactivating panel 保存宿主焦点，并在激活前重验 exact `FocusToken`。完整文本走 `Delivery.insert`；图片、文件等富内容恢复原始 pasteboard 后，只向同一 token 的精确宿主 PID 发送带内部标签的粘贴事件。
