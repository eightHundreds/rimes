...
- Keep the current Buffer catalog limited to AI Generation 2.1, Real-time Translation 2.1, Stream Input 1.3, and Capsule 0.3. Capsule owns Prompt, Memory, Password, and Skill as type-filtered local Markdown entries; password delivery requires its explicit protected prompt. My Prompt, Remarkable, and Marine Chrome are retired from current cards, configuration branches, and Buffer modes. Keep External Source Inbox independent of those retired plugins and use neutral local-pairing fixtures.
